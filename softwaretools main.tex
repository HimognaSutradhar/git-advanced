...
\begin{figure}
    \centering
    \includegraphics[width=0.4\linewidth]{makaut.png}
\end{figure}
\maketitle
...
\end{table}
\vspace{2.5cm}
\section{Lab Entry by Atanu Adhikary}
\subsection{Experiment}
\vspace{0.5cm}
\begin{table}[ht]
\centering
...
[remember picture, overlay] \draw[line width = 2pt] ($(current page.north west) + (0.5in, -0.5in)$) rectangle ($(current page.south east) + (-0.5in, 0.5in)$);
\end{tikzpicture}
\title{Lab Entry By\\[0.5cm] \large Himogna Sutradhar\\[0.5cm]
    \begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{github logo.png} % Ensure the image is correctly located
\end{figure}
\author{}
\date{}
\maketitle
...
\newpage
\begin{center}
\title{Lab Entry By\\[0.5cm] \large Sneha Chandra\\[0.5cm]
...
\begin{figure}[h!]
    \centering
            \includegraphics[width=0.5\linewidth]{input 1.jpg}
            \caption{JAVA CODE}
...
\newpage
\title{\centering \Large \textbf{5. Creating LaTeX Repository on GitHub} \\[0.5cm]}
\author{\centering \Large \textbf{Lab Entry By} \\[0.5cm] \large \textbf{Atanu Adhikary}}
\begin{document}
